...
public class TestWCMComposer extends BasePublicationTestCase {
	...
	public void testgetPaginatedContentsShouldRemoveDuplicatedNodes() throws Exception {
		...
		List<Node> l1 = new ArrayList<Node>(){{add(nodeone_fr);}};

		List<Node> l2 = new ArrayList<Node>(){{add(nodeone_en);}};

		Mockito.doReturn(iter).when(wcmComposerImplSpy).getViewableContents(Mockito.any(String.class), Mockito.any(String.class), Mockito.any(HashMap.class), Mockito.any(SessionProvider.class), Mockito.any(Boolean.class));
	...
	public void testgetContentsShouldRemoveDuplicatedNodes() throws Exception {
		...
		Mockito.doReturn(iter).when(wcmComposerImplSpy).getViewableContents(Mockito.any(String.class), Mockito.any(String.class), Mockito.any(HashMap.class), Mockito.any(SessionProvider.class), Mockito.any(Boolean.class));
		Mockito.doReturn(l1).when(wcmComposerImplSpy).getRealTranslationNodes(nodeone_en);
		Mockito.doReturn(l2).when(wcmComposerImplSpy).getRealTranslationNodes(nodeone_fr);		List<Node> nodes = wcmComposerImplSpy.getContents(workspace,folderPath, filters, sessionProvider);
